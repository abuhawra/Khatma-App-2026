...
<style>
    ...
    /* تنسيق الصف - الخلفية الخضراء تظهر عند تطبيق كلاس row-completed */
...
def get_status_squares(status):
    ...
    return "".join([f'<div class="status-box {"s-green" if i < level else "s-gray"}"></div>' for i in range(4)])
...
if current_group_id and current_group_id in db["groups"]:
    ...
    def update_status(i, key):
        ...
        save_data(db)
        st.rerun() # تحديث الصفحة لرؤية تغيير اللون
    ...
    with tab1:
        for i in range(30):
            ...
            cols[1].markdown(f"<span class='highlight-text'>{group_data['readers'][i]}</span><br>{get_status_squares(status)}", unsafe_allow_html=True)
    ...
    if st.text_input("كلمة المرور:", type="password") == "admin":
        ...
        with tab3:
            ...
            if st.button("حفظ"):
                g_info["readers"] = e_text.splitlines(); save_data(db); st.rerun()import streamlit as st
import json
import uuid
import datetime
import os
